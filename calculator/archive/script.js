...
$(document).ready(function () {
  ...
  $('#costBreakdownToggle').on('click', function() {
    const $button = $(this);
    ...
    if ($content.is(':visible')) {
      ...
      $toggleText.text('SHOW COST & SAVINGS BREAKDOWN');
      $toggleIcon.text('+');
    } else {
      // Show content
      $content.slideDown(300);
      $button.addClass('active');
      $toggleText.text('HIDE COST & SAVINGS BREAKDOWN');
      $toggleIcon.text('×');
    }
  });

  // Operating costs toggle functionality
  $('#operatingCostsToggle').on('click', function() {
    const $button = $(this);
    const $content = $('#operatingCostsContent');
    const $toggleText = $button.find('.toggle-text');
    const $toggleIcon = $button.find('.toggle-icon');
    ...
    if ($content.is(':visible')) {
      ...
      $content.slideUp(300);
      $button.removeClass('active');
      $toggleText.text('SHOW OPERATING COSTS BREAKDOWN');
      $toggleIcon.text('+');
    } else {
      // Show content
      $content.slideDown(300);
      $button.addClass('active');
      $toggleText.text('HIDE OPERATING COSTS BREAKDOWN');
      $toggleIcon.text('×');
    }
  });
